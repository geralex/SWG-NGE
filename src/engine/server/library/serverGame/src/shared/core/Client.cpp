...
namespace ClientNamespace
{
	...
	bool canManipulateObjectExceptionCheck(ServerObject const &target, int menuType, Container::ContainerErrorCode& errorCode)
	{
		if (errorCode == Container::CEC_NoPermission)
		{
			...
			{
				if (s_radialMenuPermissionOverrideTemplateItemType.count(std::make_pair(target.getTemplateCrc(), menuType)) > 0)
				{
					errorCode = Container::CEC_Success;
					return true;
				}
			}
			else if (s_radialMenuPermissionOverrideStaticItemType.count(std::make_pair(objectStaticName, menuType)) > 0)
			{
				errorCode = Container::CEC_Success;
				return true;
			}
...
Client::ClientMessage::ClientMessage(const std::string & src) :
MessageDispatch::MessageBase(src.c_str())
{
}
...
Client::Client(ConnectionServerConnection & connection, const NetworkId & characterObjectId, const std::string& accountName, const std::string& ipAddr, bool isSecure, bool isSkipLoadScreen, unsigned int stationId, std::set<NetworkId> const & observedObjects, uint32 gameFeatures, uint32 subscriptionFeatures, AccountFeatureIdList const & accountFeatureIds, unsigned int entitlementTotalTime, unsigned int entitlementEntitledTime, unsigned int entitlementTotalTimeSinceLastLogin, unsigned int entitlementEntitledTimeSinceLastLogin, int buddyPoints, std::vector<std::pair<NetworkId, std::string> > const & consumedRewardEvents, std::vector<std::pair<NetworkId, std::string> > const & claimedRewardItems, bool usingAdminLogin, CombatDataTable::CombatSpamFilterType combatSpamFilter, int combatSpamRangeSquaredFilter, int furnitureRotationDegree, bool hasUnoccupiedJediSlot, bool isJediSlotCharacter, bool sendToStarport ) :
	...
	m_isReady(false),
	m_isSecure(isSecure),
	m_isSkipLoadScreen(isSkipLoadScreen),
	...
	m_previousObservedObjects(observedObjects),
	m_syncUIs(),
	...
	m_sendToStarport( sendToStarport )
{
	...
	if (obj)
	{
			...
			if ((buildingObject) && (!buildingObject->isInWorld()))
			{
				...
				if (cby)
				{
					...
					if (!ServerWorld::isSpaceScene())
					{
						...
						if (obj->getObjVars().getItem("building_ejection_point", ejectionPoint) && (ServerWorld::getSceneId() == ejectionPoint.scene) && (!ejectionPoint.cell.isValid()))
						{
			...
			else if (containerObject->isAuthoritative())
			{
				...
				if (ContainerInterface::getContainedByObject(*obj) != containerObject)
				{
					...
					if (!ServerWorld::isSpaceScene())
					{
						...
						if (obj->getObjVars().getItem("building_ejection_point", ejectionPoint) && (ServerWorld::getSceneId() == ejectionPoint.scene) && (!ejectionPoint.cell.isValid()))
						{
							DEBUG_REPORT_LOG(true,("Ejecting player %s from building %s, because could not put player back in building\n",characterObjectId.getValueString().c_str(), (buildingObject ? buildingObject->getNetworkId().getValueString().c_str() : containerObject->getNetworkId().getValueString().c_str())));
							LOG("CustomerService",("Login:  Ejecting player %s from building %s, because could not put player back in building",characterObjectId.getValueString().c_str(), (buildingObject ? buildingObject->getNetworkId().getValueString().c_str() : containerObject->getNetworkId().getValueString().c_str())));
				...
				else
				{
					...
					if (buildingObj && buildingObj->asServerObject() && buildingObj->asServerObject()->asBuildingObject())
		...
		if (!containerObject || (containerObject->isInWorld() && containerObject->getContainerProperty()->isContentItemExposedWith(*obj)))
			obj->addToWorld();
...
{
	DEBUG_FATAL(newCharacter.getNetworkId() != m_primaryControlledObject, ("Tried to assume control of the wrong object?"));
	...
	NameManager::getInstance().addPlayer(newCharacter.getNetworkId(), (playerObject ? playerObject->getStationId() : static_cast<uint32>(m_stationId)), firstName, fullName, 0, Os::getRealSystemTime(), true);
	...
	ControlAssumed const controlMessage(
		m_primaryControlledObject,
		ConfigServerGame::getGroundScene(),
		m_isSkipLoadScreen,
		newCharacter.getTransform_o2w().getPosition_p(),
		newCharacter.getObjectFrameK_w().theta(),
		newCharacter.getSharedTemplateName(),
		static_cast<int64>(ServerClock::getInstance().getGameTimeSeconds()));
		...
		else if(message.isType("ObjControllerMessage"))
		{
			...
			if (allowFromClient) {
				...
				else if (isGod())
				{
				 ...
				 LOG("hackz", ("Unauthorized Controller Message: Player %s at %s sent an unauthorized controller message %d for object %s", PlayerObject::getAccountDescription(getCharacterObjectId()).c_str(), getIpAddress().c_str(), o.getMessage(), o.getNetworkId().getValueString().c_str()));
...
void Client::resetIdleTimeAfterCharacterTransfer()
{
	...
	if ((currentGameTime + ConfigServerGame::getIdleLogoutTimeAfterCharacterTransferSec()) > ConfigServerGame::getIdleLogoutTimeSec())
	{
		m_lastNonIdleTime = currentGameTime + ConfigServerGame::getIdleLogoutTimeAfterCharacterTransferSec() - ConfigServerGame::getIdleLogoutTimeSec();
...
{
	return ServerClock::getInstance().getGameTimeSeconds()-m_lastNonIdleTime > ConfigServerGame::getIdleLogoutTimeSec();
}
...
bool Client::shouldReceiveCombatSpam(NetworkId const & attacker, Vector const & attackerPosition_w, NetworkId const & defender, Vector const & defenderPosition_w) const
{
	...
	if (primaryControlledObject)
	{
		...
		if ((attacker.isValid() && (playerPosition.magnitudeBetweenSquared(attackerPosition_w) <= m_combatSpamRangeSquaredFilter)) || (defender.isValid() && (playerPosition.magnitudeBetweenSquared(defenderPosition_w) <= m_combatSpamRangeSquaredFilter)))
			return true;
	}
